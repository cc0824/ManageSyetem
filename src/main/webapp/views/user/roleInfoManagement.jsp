	...
	$(function() {
		...
		function build_userWithRole_table(result){
			...
			$.each(userWithRoles,function(index,item){
				...
				var roles=item.roleList;
				var userRoleNameTd =$("<td></td>")
				$.each(roles,function(index,item){
					userRoleNameTd.append(item.roleName);				
		...
		$(document).on("click",".edit_btn",function(){
			...
			$(".selectpicker").empty();
			//2、查出角色信息
			getSelectRole($(this).attr("edit-id"));			
			//3、把角色的id传递给模态框的更新按钮
		...
		function getSelectRole(id){
			$.ajax({
				url:"${APP_PATH}/role/getAllRole.do",
				data:"id="+id,
				type:"GET",
				success:function(result){
					console.log(result);
					//拿到返回的数据
					var roleNames=result.extend.roleList;
					var selectTemp=$("<select></select>").addClass("selectpicker").attr({ "data-live-search":"true"});
					$.each(roleNames,function(index,item){
						$("#selector").append("<option value='"+roleNames[index].roleId+"'>"+roleNames[index].roleName+"</option>");
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<!-- 使用水平排列的表单 -->
						<form class="form-horizontal" id="roleUpdateForm" >
...
<!-- 									</select> -->
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-primary" id="role_update_btn">修改</button>
