...
<script type="text/javascript">
	$(function() {
		...
		function to_page(pn){
			$.ajax({
				url:"${APP_PATH}/user/getAllUserList.do",
				data:"pn="+pn,//pn=指定的pn
				type:"GET",
		...
		$(document).on("click",".edit_btn",function(){
			//2、查出用户信息
			getSelectUser($(this).attr("edit-id"));			
			//3、把用户的id传递给模态框的更新按钮
		...
		function getSelectUser(id){
			$.ajax({
				url:"${APP_PATH}/user/getSelectUserById.do",
				data:"id="+id,
				type:"GET",
				success:function(result){
					console.log(result);
					//给input框赋值
					$("#userName_update_input").val(result.userName);
					$("#userPassword_update_input").val(result.userPassword);
								...
								<div class="col-xs-9">
									<!-- 不想让商品信息被修改  将input替换成静态控件 -->
 									<input type="text" name="userName" class="form-control"
										id="userName_update_input" placeholder="商品名称">
									<span class="help-block"></span>
								</div>
							</div>
							<!-- 修改用户密码的div -->
							<div class="form-group">
								...
								<div class="col-xs-9">
									<input type="text" name="userPassword" class="form-control"
										id="userPassword_update_input" placeholder="商品成本"> 
									<span class="help-block"></span>
								</div>
							</div>
						</form>
